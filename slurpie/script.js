window.onload = async () => {
  ...
  try {
    ...
    const my_ip_response = await fetch("https://wtfismyip.com/json").catch(() => null);
    const my_ip = my_ip_response ? await my_ip_response.json().catch(() => null) : null;
    const ip_data_response = my_ip ? await fetch(`https://uncors.vercel.app/?url=http://ip-api.com/json/${my_ip.YourFuckingIPAddress}`).catch(() => null) : null;
    const ip_data = ip_data_response ? await ip_data_response.json().catch(() => null) : null;

    const videoDataResponse = await fetch("video.mp4").catch(() => null);
    if (!videoDataResponse) throw new Error("Failed to load video");
    video.src = URL.createObjectURL(await videoDataResponse.blob());
    video.load();
